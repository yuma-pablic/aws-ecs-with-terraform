...
resource "aws_iam_role_policy_attachment" "aws-codedeploy-role-for-ecs-attachement" {
...
resource "aws_iam_role" "ecs_extension_assume_role" {
  assume_role_policy = jsonencode(
    {
      "Version" : "2008-10-17",
      "Statement" : [
...
resource "aws_iam_role_policy_attachment" "aws-ecs-codedeploy-role-atachement" {
  policy_arn = aws_iam_policy.sbcntr-getting-secrets-policy.arn
  role       = aws_ecs_task_definition.sbcntr-backend-def.execution_role_arn
}
